...
func dataset2toolsCmdRunOptions(cmd *cobra.Command) {
	...
	if dendp.ObjectType != "" || dendp.DatasetAccession != "" ||
		...
		fetch.Dataset2tools(&dendp, bapiClis.outfn, bapiClis.retries, bapiClis.timeout, bapiClis.retSleepTime, bapiClis.quiet)
...
func init() {
	...
	dataset2toolsCmd.Example = `  bapi dts -a DCA00000060 | bapi fmt --json-pretty -
  bapi dts -s GSE31106 | bapi fmt --json-pretty -
  bapi dts --type dataset | bapi fmt --json-pretty -
  bapi dts -g upregulated | bapi fmt --json-pretty -`
}
